...
def execute_rotations(rotation_plan: dict) -> None:
    """Execute every rotation in *rotation_plan* {cell: count}."""
    total = sum(rotation_plan.values())
    print(f"\nExecuting {total} rotation(s) across {len(rotation_plan)} cell(s)")

    for cell, count in rotation_plan.items():
        print(f"\n  Cell {cell}: {count} rotation(s)")
